...
describe('fraction training capabilities', () => {
  it('offers depth, length, and a timed personal challenge', () => {
    ...
    expect(FRACTION_TRAINING_CAPABILITIES.supportsChallenge).toBe(true);
    // The shortest practice length stays practice-only, same reasoning as
    // Multiplication Tables: avoid noisy, too-short challenge records.
    expect(FRACTION_TRAINING_CAPABILITIES.challengeQuestionCounts).toEqual([10, 20]);
    // A pace personal best requires a flawless round, so a faster-but-wrong
    // run can never register as a new speed record.
    expect(FRACTION_TRAINING_CAPABILITIES.paceRecordMinAccuracy).toBe(1);
  });
